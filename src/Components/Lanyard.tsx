...
const Lanyard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_LANYARD_API_URL}/users/${import.meta.env.VITE_LANYARD_USER_ID}`,
          {
            headers: {
              Authorization: import.meta.env.VITE_LANYARD_API_KEY!,
            },
          }
        );
    ...
    const socket = new WebSocket(import.meta.env.VITE_LANYARD_SOCKET_URL!);
    ...
    socket.onopen = () => {
      ...
      socket.send(
        JSON.stringify({
          op: 2,
          d: {
            subscribe_to_id: import.meta.env.VITE_LANYARD_USER_ID!,
          },
        })
      );
    };
